...
class LowestCostFirstQueue(Queue):
    """A queue in which the element with the minimum cost is returned first"""
...
class HeuristicQueue(Queue):
    """A queue in which the element with the minimum cost and heuristic sum is returned first"""
    ...
    def extend(self, items):
        self.A.extend(items)
        return self.sort_by_heuristic()

    def pop(self):
        return self.A.pop()

    def sort_by_heuristic(self):
        return self.A.sort(key=lambda x: x.path_cost + self.problem.h(x), reverse=True)
